...
import { Search, MessageSquare, Send, Phone, MoreVertical, Check, CheckCheck, Building, User, Paperclip, X, FileText, Image } from 'lucide-react';
interface AuthUser {
  id: string;
  email: string;
  name: string;
...
export default function UserDashboard() {
  ...
  // Poll /api/chat every second — works across any browser origin
  useEffect(() => {
    const poll = async () => {
      try {
        ...
        const userConvs: Conversation[] = (data.conversations || []).filter((c: Conversation) => c.userId === user.id);
        ...
        if (!conv && userConvs.length > 0) {
          ...
          selectedConvRef.current = userConvs[0];
        }
          ...
          {selectedConversation ? (
            <>
              <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center flex-shrink-0">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="font-semibold text-gray-900 text-sm">{selectedConversation.companyName}</p>
                    <p className="text-[10px] text-gray-400 uppercase">Company Admin</p>
                  </div>
                </div>
                ...
                {messages.map((msg) => (
                  <div key={msg.id} className={`flex ${msg.senderType === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[70%] rounded-2xl shadow-sm text-sm overflow-hidden ${
                        ...
                        <div className="p-2">
                          {msg.attachment.type.startsWith('image/') ? (
                            ...
                            <a href={msg.attachment.url} download={msg.attachment.name} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs ${
                              msg.senderType === 'user' ? 'bg-pink-500/40 hover:bg-pink-500/60' : 'bg-gray-100 hover:bg-gray-200'
                            }`}>
                      ...
                      <div className={`flex items-center justify-end gap-1 px-4 pb-2 text-[10px] ${msg.senderType === 'user' ? 'text-pink-100' : 'text-gray-400'}`}>
                        <span>{formatTime(msg.timestamp)}</span>
                        {msg.senderType === 'user' && (msg.status === 'read' ? <CheckCheck className="w-3 h-3" /> : <Check className="w-3 h-3" />)}
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-400">Messages from companies will appear here</p>
              </div>
            </div>
          )}
        </div>
